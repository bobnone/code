...
struct WeightedEdge
{
	...
	struct WeightedGraphNode* pFrom;
	struct WeightedGraphNode* pTo;
	// Weight of this edge
	float mWeight;
};
...
struct GBFSScratch
{
	const WeightedEdge* pParentEdge = nullptr;
...
bool AStar(const WeightedGraph& g, const WeightedGraphNode* start, const WeightedGraphNode* goal, AStarMap& outMap)
{
	...
	do
	{
		...
		{
			const WeightedGraphNode* neighbor = edge->pTo;
...
bool GBFS(const WeightedGraph& g, const WeightedGraphNode* start, const WeightedGraphNode* goal, GBFSMap& outMap)
{
	...
	do
	{
		...
		{
			// Get scratch data for this node
			GBFSScratch& data = outMap[edge->pTo];
			...
			if (!data.mInClosedSet)
			{
				...
				if (!data.mInOpenSet)
				{
					...
					data.mHeuristic = ComputeHeuristic(edge->pTo, goal);
					data.mInOpenSet = true;
					openSet.emplace_back(edge->pTo);
				}
			}
		}
		// If open set is empty, all possible paths are exhausted
...
float GetScore(const GameState& state)
{
	...
	if (((state.mBoard[0][0] == state.mBoard[1][1]) && (state.mBoard[1][1] == state.mBoard[2][2])) || ((state.mBoard[2][0] == state.mBoard[1][1]) && (state.mBoard[1][1] == state.mBoard[0][2])))
	{
